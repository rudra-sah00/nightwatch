...
Channels originating from Server 1 are considered premium and therefore are protected against direct browser scraping by forcing them into the Tauri Desktop application bounds.
1. The frontend (`live/[id]/page.tsx`) checks if a stream's `match.playPath` starts with `s1://` or `match.id` starts with `s1:`.
2. It strictly validates whether the app is running inside the Tauri webview window. 
...
### LiveBridge (Tauri Proxy)
Relying on a standard web browser `video` tag is insufficient for Server 1 because complex authentications, cookie headers, or bespoke token-fetching require Rust-level network interception. 
- The Tauri frontend calls `desktopBridge.startLiveBridge({ url, channelId, referer })`.
- Within the OS sandbox, `src-tauri/src/commands/live_bridge.rs` handles the full user-agent lifecycle.
- It dynamically intercepts HTTP streams and parses them through a persistent local proxy loop (`http://localhost:{PROXY_PORT}`).
- Once resolved, the Tauri event listener signals the React frontend, passing back the local `result.hlsUrl` which the `WatchLivePlayer` mounts securely without risking CORS blocks or external token expiration.
