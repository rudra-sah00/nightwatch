  ...
  return (
    <div className="flex flex-col h-full max-h-full overflow-hidden">
      <div className="space-y-3 flex-none mb-4">
        <h4 className="text-sm font-black text-muted-foreground font-headline uppercase tracking-widest flex items-center justify-between">
        ...
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground stroke-[3px]" />
          <input
            placeholder={t('soundboard.searchPlaceholder')}
            className="w-full pl-9 bg-muted/50 border border-border h-10 text-sm font-bold font-headline tracking-wide text-foreground placeholder:text-muted-foreground rounded-lg outline-none focus:border-foreground/30 transition-colors"
            value={searchQuery}
        ...
        <div className="flex flex-col gap-1 pb-4">
          {sounds.map((sound) => (
            <button
              ...
              className="h-auto py-2 px-3 flex items-center gap-2 justify-start text-foreground/80 hover:text-foreground hover:bg-muted transition-colors group relative overflow-hidden rounded-lg"
          ...
          {loading ? (
            <div className="py-4 flex justify-center">
              <Loader2 className="w-5 h-5 animate-spin text-muted-foreground stroke-[3px]" />
            </div>
          ) : null}
