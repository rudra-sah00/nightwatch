...
export function useWatchProgress({
    ...
    const updateProgress = useCallback(() => {
        ...
        if (!duration || isNaN(duration) || currentTime === 0) return;

        const socket = getSocket();
        if (socket?.connected) {
            const payload = {
                contentId: metadata.movieId,
                ...
                episodeId: metadata.type === 'series' && metadata.season && metadata.episode
            ...
            socket.emit('watch:update_progress', payload, (res: any) => {
                if (!res?.success) {
                    console.warn('Failed to update progress:', res?.error);
                }
            });
        }
    }, [metadata, videoRef]);
    ...
    useEffect(() => {
        ...
        const loadProgress = () => {
            socket.emit('watch:get_progress', {
                contentId: metadata.movieId,
                episodeId: metadata.type === 'series' && metadata.season && metadata.episode
                    ? `${metadata.season}-${metadata.episode}`
                    : undefined
    ...
    }, [metadata.movieId, metadata.season, metadata.episode, metadata.type, onProgressLoaded]);
