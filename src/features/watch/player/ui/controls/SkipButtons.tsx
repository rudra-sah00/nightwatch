...
const skipBackIcon = <SkipBack className="w-8 h-8 text-white stroke-[3px]" />;
const skipForwardIcon = (
  <SkipForward className="w-8 h-8 text-white stroke-[3px]" />
);
...
export function SkipButton({
  ...
  return (
    <button
      type="button"
      onClick={onSkip}
      className="p-1 text-white/80 hover:text-white transition-colors duration-200 group relative flex items-center justify-center"
      title={
        direction === 'back'
          ? t('skipBack', { seconds })
    ...
    >
      ...
      <span className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 text-[10px] font-black font-headline text-white/70 px-1">
        {seconds}
      </span>
    </button>
  );
...
export function SeekIndicator({
  ...
  return (
    ...
    >
      ...
      <span className="text-white px-3 py-1 font-black font-headline uppercase tracking-widest text-sm">
        {seconds}s
      </span>
    </div>
  );
