...
import { PlayerAction, Quality, AudioTrack } from './types';
            ...
            if (Hls.isSupported()) {
                ...
                hls.on(Hls.Events.MANIFEST_PARSED, (_, data) => {
                    ...
                    if (data.audioTracks && data.audioTracks.length > 0) {
                        ...
                        if (defaultTrack) {
                            dispatch({ type: 'SET_CURRENT_AUDIO_TRACK', trackId: defaultTrack.id });
                        }
                    }
                ...
                hls.on(Hls.Events.AUDIO_TRACK_SWITCHED, (_, data) => {
                    dispatch({ type: 'SET_CURRENT_AUDIO_TRACK', trackId: String(data.id) });
                });
                ...
                hls.on(Hls.Events.LEVEL_SWITCHED, (_, data) => {
                    ...
                    if (level) {
                        dispatch({ type: 'SET_CURRENT_QUALITY', quality: `${level.height}p` });
                    }
                });
                    ...
                    if (data.fatal) {
                        switch (data.type) {
                            ...
                            case Hls.ErrorTypes.MEDIA_ERROR:
                                ...
                                break;
                            default:
                                dispatch({ type: 'SET_ERROR', error: 'Playback error occurred' });
                                hls.destroy();
                                break;
                        }
                    }
