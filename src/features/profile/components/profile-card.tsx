...
import { User as UserIcon, Mail, Calendar, Camera, Loader2, X } from 'lucide-react';
...
import { WatchActivity } from '../types';
...
export function ProfileCard() {
    ...
    useEffect(() => {
        getWatchActivity().then(setActivity).catch(console.error);
    }, []);
    ...
    const formattedJoinDate = useMemo(() =>
        userCreatedAtDate
            ? userCreatedAtDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })
            : 'Unknown',
        [userCreatedAtDate]);
                    ...
                    <aside className="lg:sticky lg:top-24">
                        <div className="rounded-2xl border bg-card/60 backdrop-blur-xl p-6 lg:p-8 shadow-sm flex flex-col items-center text-center space-y-5">
                            ...
                            <div className="w-full flex items-center justify-center pb-2 border-b border-border/10">
                                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Profile Card</span>
                            </div>
                            ...
                            <div className="relative group">
                                <div className="w-32 h-32 lg:w-36 lg:h-36 rounded-full p-1 border-2 border-border/30 bg-background shadow-xl">
                                    <Avatar
                                        ...
                                        fallback={<UserIcon className="w-16 h-16 lg:w-20 lg:h-20 text-muted-foreground/50" />}
                                    />
                                </div>
                                <div
                            ...
                            <div className="space-y-1 w-full">
                                <h1 className="text-xl lg:text-2xl font-bold tracking-tight truncate">{user.name}</h1>
                                {user.username && (
