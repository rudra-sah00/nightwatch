...
function parseDevice(ua: string): { icon: typeof Monitor; label: string } {
  ...
  if (lower === 'desktop app') return { icon: Monitor, label: 'Desktop App' };
  if (
  ...
  if (lower.includes('electron'))
    return { icon: Monitor, label: 'Desktop App' };
  if (lower.includes('chrome')) return { icon: Globe, label: 'Chrome' };
  if (lower.includes('firefox')) return { icon: Globe, label: 'Firefox' };
  if (lower.includes('safari')) return { icon: Globe, label: 'Safari' };
    ...
    <section className="bg-card text-card-foreground border border-border rounded-xl shadow-sm p-8">
      ...
      <div className="space-y-3 max-w-2xl">
        {sessions.map((session) => {
          const { icon: Icon, label } = parseDevice(session.device);
          return (
            <div
            ...
            >
              ...
              {!session.isCurrent ? (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-destructive border border-destructive/30 rounded-lg hover:bg-destructive/10 active:scale-95 transition-all disabled:opacity-50"
                >
      ...
      {sessions.filter((s) => !s.isCurrent).length > 1 ? (
        <button
          ...
          className="mt-6 px-4 py-2 text-sm font-medium text-destructive border border-destructive/30 rounded-lg hover:bg-destructive/10 active:scale-95 transition-all"
        >
