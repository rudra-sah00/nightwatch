...
import { SearchHistory, SearchResult, ShowDetails, PlayResponse } from './types';
...
export async function searchContent(query: string): Promise<SearchResult[]> {
    const { results } = await apiFetch<{ results: SearchResult[] }>(`/api/video/search?q=${encodeURIComponent(query)}`);
    return results;
}
...
export async function getSeriesEpisodes(seriesId: string, startSeasonId?: string): Promise<{ episodes: any[]; totalEpisodes: number }> {
...
export async function getPlayStatus(): Promise<{ status: string; queueLength: number; isProcessing: boolean }> {
