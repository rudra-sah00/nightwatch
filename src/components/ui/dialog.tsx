  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          'bg-white text-foreground data-[state=open]:animate-in data-[state=closed]:animate-out motion-reduce:data-[state=open]:animate-none motion-reduce:data-[state=closed]:animate-none data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-none border-[4px] border-border p-8 shadow-none  duration-200 outline-none sm:max-w-lg',
          className,
        )}
        {...props}
      >
        ...
        {showCloseButton ? (
          <DialogPrimitive.Close
            ...
            className="absolute top-4 right-4 bg-white border-[3px] border-border p-1.5 text-foreground hover:bg-[#e63b2e] hover:text-white transition-[background-color,color,border-color,opacity,transform]"
          >
...
function DialogDescription({
  ...
  return (
    <DialogPrimitive.Description
      ...
      className={cn(
        'text-[#4a4a4a] text-sm font-headline font-bold uppercase tracking-widest',
        className,
      )}
      {...props}
    />
