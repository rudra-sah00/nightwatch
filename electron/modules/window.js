...
class AppWindow {
  ...
  create() {
    ...
    session.defaultSession.setPermissionRequestHandler((webContents, permission, callback) => {
      ...
      const allowedPermissions = ['media', 'camera', 'microphone', 'notifications', 'clipboard-sanitized-write'];
    ...
    this.mainWindow.webContents.on('will-navigate', (event, url) => {
      ...
      if (!url.startsWith('https://watch.rudrasahoo.live') && !url.startsWith('watch-rudra://')) {
        event.preventDefault();
        shell.openExternal(url);
      }
    });
    ...
    this.mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
      ...
      if (errorCode >= -199 && errorCode <= -100) {
        ...
        this.mainWindow.loadFile(require('path').join(__dirname, '../offline.html'));
      }
    });
