...
const withSerwist = withSerwistInit({
  // Source: the SW file we authored in src/app/sw.ts
  swSrc: 'src/app/sw.ts',
  // Destination: compiled output served by Next.js at /sw.js
  swDest: 'public/sw.js',
  // Disable SW in development to avoid caching stale responses while coding
  disable: process.env.NODE_ENV === 'development',
  // Cache all navigations so every route is available offline after first visit
  cacheOnNavigation: true,
  // Reload all open tabs immediately when SW updates (no "waiting" state)
  reloadOnOnline: true,
});
